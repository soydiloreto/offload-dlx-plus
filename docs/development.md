...
When `make env` finishes, open <http://localhost:8888>. Log in with `admin` / `password`. The plugin is already mounted at `wp-content/plugins/offload-plus/` — activate it from the **Plugins** screen.
...
| `make i18n` | `wp i18n make-pot` — extract the translatable strings into `build/offload-plus.pot`. |
...
| `make deploy-test` | Sync the current branch into `~/repos/mug-website-v2/wp-content/plugins/offload-plus/` for a smoke test on a real site (the script lives at `~/.local/bin/offload-plus-deploy-to-mug` by default — override with `DEPLOY_SCRIPT=/path/to/script`). |
...
If you'd rather use your own WordPress setup instead of `wp-env`, clone this repo directly into `wp-content/plugins/offload-plus/` of your existing WordPress install. The plugin has no build step — it runs straight from source.
